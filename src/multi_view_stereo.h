...
public:
    ...
    void cost_aggregation(Image* const ref_image, bool debug_plot = false);

    void init_aggregate_pixel(PixelPoint& pixel_point);
  
    
    void update_uncertainty(Image* const ref_image, bool debug_plot = false);
...
protected:
    ...
    void cost_aggregation_left_right(Image* const ref_image, bool is_forward, bool debug_plot);
    void cost_aggregation_up_down(Image* const ref_image, bool is_forward, bool debug_plot);
    void cost_aggregation_from_last_pixel(const PixelPoint& pixel_point_in_last_pos, PixelPoint& pixel_point);
    ...
    void pixel_depth_estimation(EpipolarSegment& epipolar_segment, const Eigen::Vector3f& Kt, const Eigen::Vector3f& KRKi_uv_homo, const Eigen::Vector2f& uv_best_match_minus, const Eigen::Vector2f& uv_best_match_plus);
